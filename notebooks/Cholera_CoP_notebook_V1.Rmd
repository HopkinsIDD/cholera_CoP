...
# For all participants, find individuals who have been lost to follow-up (i.e., no information for >24 days)
...
over24 <- latest_check %>% 
  ...
  pivot_wider(names_from = vxf_arm,
              values_from = count) %>% 
  rename(Group = past24) %>% 
  mutate(Description = "Participants with >24 days delayed follow-up")

# Summary table
study_summary <- rbind(subtotal, age_count, sex_count, completion, over24) %>% 
...
Table 3 displays the status of participants with delayed follow-up visits (>24 days). The `min_date` column shows the date of their enrollment and the `max_date` column shows the date of their latest follow-up visit (across all forms). The `pst_column` shows whether these individuals have filled out the Subject Status Form to mark completion or withdrawal from the study.

```{r delayed-participants}
# Show participants with >24 days delayed follow-up
latest_check %>% 
  filter(past24 == TRUE) %>% 
  select(record_id, scr_elig_status, scr_date, scr_age, scr_sex,
         vxf_arm, pst_complete, min_date, max_date, current_date, time_since_last, past24) %>% 
...
phone_summary <- cholera_phone_dat %>% 
...
Table 5 and Figure 1 will show the prospectively scheduled visits for each eligible study participants and whether those visits have been completed, still pending, or are overdue. If participants have an overdue visit but have had a more recent visit (≤24 days from today's date), they will not appear in Table 3 above.
...
merge_events <- right_join(x = cholera_cop_dat, y = all_events) %>%
  ...
  fill(scr_date, scr_elig_status, vxf_arm, .direction = "down") %>%
  filter(scr_elig_status == "Eligible") %>%
...
```{r cohort-plot, fig.height = 9, fig.width = 9}
# Cohort plot
cohort_plot <- merge_events %>% 
  ...
  geom_point(aes(x = merge_date, y = record_id, shape = status_group, color = status_group),
             size = 0.8) +
  ...
  scale_color_manual(name = "Status",
                     values = c("black", "grey50", "grey50", "grey50")) +
