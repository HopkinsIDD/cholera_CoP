...
latest_check <- cholera_cop_dat %>%
  ...
  mutate(past14 = time_since_last > 14) %>%
...
over14 <- latest_check %>% 
  group_by(vxf_arm, past14) %>% 
  summarize(count = n()) %>% 
  pivot_wider(names_from = vxf_arm,
              values_from = count) %>% 
  rename(Group = past14) %>% 
  mutate(Description = "Participants with >14 days delayed follow-up")

# Summary table
study_summary <- rbind(subtotal, age_count, sex_count, completion, over14) %>% 
...
Table 2 shows that among the `r total[total$scr_elig_status == "Eligible",]$count` eligible study participants, they have been divided into the two arms of the study based on the vaccine administered. Participant demographic data (age and sex) are summarized by study arm. Participants that have completed the study or withdrawn early will complete a Subject Status Form. This table also tracks the individuals that have delayed follow-up (>14 days) based on their scheduled visits from their date of enrollment. At present, there are `r study_summary[study_summary$Description == "Participants with >14 days delayed follow-up" & study_summary$Group == TRUE,]$Total` participants with delayed follow-up visits. These individuals are listed in Table 3.
...
Table 3 displays the status of participants with delayed follow-up visits (>14 days). The `min_date` column shows the date of their enrollment and the `max_date` column shows the date of their latest follow-up visit (across all forms). The `pst_column` shows whether these individuals have filled out the Subject Status Form to mark completion or withdrawal from the study.

```{r delayed-participants}
# Show participants with >14 days delayed follow-up
latest_check %>% 
  filter(past14 == TRUE) %>% 
  select(record_id, scr_elig_status, scr_date, scr_age, scr_sex,
         vxf_arm, pst_complete, min_date, max_date, current_date, time_since_last, past14) %>% 
  kable(caption = "Table 3: Participants with delayed follow-up (>14 days)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
At present (`r Sys.Date()`), there have been `r nrow(cholera_phone_dat_alt)` phone calls. However, phone calls were not scheduled to begin until 51 days past subject enrollment, so the earliest phone calls for the first enrollees should occur on `r min(cholera_cop_dat$scr_date, na.rm = TRUE) + 51`.
...
Once there are phone call data to summarize, we will summarize the number calls made each week and the number calls with diarrhea report.
...
Table 4 and Figure 1 will show the prospectively scheduled visits for each eligible study participants and whether those visits have been completed, still pending, or are overdue. If participants have an overdue visit but have had a more recent visit (≤14 days from today's date), they will not appear in Table 3 above.
...
merge_events <- right_join(x = cholera_cop_dat, y = all_events) %>%
  ...
  mutate(
    ...
    status_group = case_when(str_detect(status, "complete") ~ "complete",
                             str_detect(status, "pending") ~ "pending",
...
merge_status_table <- merge_events %>% 
  ...
  mutate(status = cell_spec(status, "html",
                            color = case_when(str_detect(status, "complete") ~ "green",
                                              ...
                                              str_detect(status, "late") ~ "red"))
  ) %>% 
  pivot_wider(names_from = nickname, values_from = status)
kable(merge_status_table, escape = FALSE,  caption = "Table 4: Record ID status")
...
Table 5 summarizes the total missed visits by study vaccine arm and study timepoint.
...
merge_events %>% 
  ...
  kable(caption = "Table 5: Record ID status summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
```{r cohort-plot, fig.height = 7, fig.width = 9}
# Cohort plot
cohort_plot <- merge_events %>% 
  ...
  geom_rect(aes(
              ...
              ymin = record_id - 0.33, ymax = record_id + 0.33,
              fill = factor(days_past))) +
  geom_point(aes(x = merge_date, y = record_id, shape = status_group),
             size = 0.8) +
  scale_x_date(name = "Prospective visit date",
               date_breaks = "2 months",
               date_labels = "%Y %b") +
  scale_y_continuous(name = "Participant ID",
                     breaks = seq(0, sum(total$count), 5)) +
